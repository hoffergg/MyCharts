package lecho.lib.hellocharts.samples.lib.renderer;
...
import lecho.lib.hellocharts.samples.lib.model.SelectedValue.SelectedValueType;
...
public class ColumnChartRenderer extends AbstractChartRenderer {
    ...
    private void drawSubcolumn(Canvas canvas, Column column, SubcolumnValue columnValue, boolean isStacked,boolean isTop) {
        ...
        if(isTop) {
            ...
            paint.setTextSize(ChartUtils.sp2px(scaledDensity, Axis.DEFAULT_TEXT_SIZE_SP));
            ...
            canvas.drawCircle(drawRect.left + drawRect.width()/2, drawRect.top,7,circlePaint);
            canvas.drawText(String.valueOf((int)column.getValueCount()), drawRect.left + drawRect.width()/2, drawRect.top-20, paint);
        }
    }
    ...
    private void highlightSubcolumn(Canvas canvas, Column column, SubcolumnValue columnValue, int valueIndex,
        ...
        if (selectedValue.getSecondIndex() == valueIndex) {
            ...
            if (column.hasLabels() || column.hasLabelsOnlyForSelected()) {
                //drawBubble(canvas, column, columnValue, isStacked, labelOffset);
                drawLabel(canvas, column, columnValue, isStacked, labelOffset);
            }
        }
    }
    ...
    private void drawBubble(Canvas canvas, Column column, SubcolumnValue columnValue, boolean isStacked, float offset){
        ...
        Drawable drawable = mContext.getResources().getDrawable(R.drawable.graph_flag_map_s);
        drawable.setBounds((int)left,(int)right,(int)top,(int)bottom);
